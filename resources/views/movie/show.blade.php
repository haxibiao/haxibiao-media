...
@section('title')
{{ seo_site_name() }} -《{{ $movie->name }}》-超清中文在线观看-{{$movie->type}}
@endsection
@section('keywords')
{{ $movie->name }}在线观看，{{ $movie->name }}超清中文字幕，{{ $movie->name }}手机观看
@endsection
@section('description')
在线观看{{ $movie->name }}，超清中字在线播放手机观看，{{ $movie->name }}剧情：{{ $movie->introduction }}
@endsection
